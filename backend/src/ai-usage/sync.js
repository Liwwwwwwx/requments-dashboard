...
const { appendSnapshot, readAccounts } = require("./store");
...
async function syncAccount(rootDir, accountId) {
  ...
  if (account.enabled === false) throw new Error("ACCOUNT_DISABLED");
  if (!account.baseUrl) throw new Error("Base URL 不能为空");
  if (!account.apiKey) throw new Error("API Key 不能为空");

  const syncOnce = async () => {
    if (account.provider === "deepseek") {
      const url = withBaseUrl(providerRootUrl(account.baseUrl), "user/balance");
      const body = await fetchProviderJson(url, account);
      return { normalized: normalizeSyncSnapshot(account, body, url), url };
    }
    if (account.provider === "kimi") {
      const url = String(account.baseUrl || "").includes("/coding/")
    ...
    if (account.provider === "minimax") {
      ...
      return {
        normalized: normalizeSyncSnapshot(account, result.body, result.url),
        url: result.url
      };
    }
    throw new Error("当前账号暂不支持自动同步，请用快照手动记录或导入 CSV");
  ...
  const isBalanceValid = (normalized) => {
    if (account.provider !== "deepseek") return true;
    const v = Number(normalized.balanceAmount);
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (isBalanceValid(result.normalized)) {
        const snapshot = appendSnapshot(rootDir, result.normalized);
        return { snapshot, syncUrl: result.url };
      }
      lastError = new Error(`DeepSeek 余额返回 ${result.normalized.balanceAmount}`);
    } catch (err) {
      lastError = err;
    }
  ...
  const reason = lastError?.message || "未知";
  const balanceText = lastNormalized ? `（余额=${lastNormalized.balanceAmount}）` : "";
  throw new Error(`DeepSeek 余额连续 ${MAX_ATTEMPTS} 次无效${balanceText}：${reason}`);
}

async function syncAllAccounts(rootDir) {
  const accounts = readAccounts(rootDir).filter((account) => account.enabled !== false);
  const results = [];
  for (const account of accounts) {
    try {
      results.push({ accountId: account.id, ok: true, result: await syncAccount(rootDir, account.id) });
    } catch (err) {
      results.push({ accountId: account.id, ok: false, error: err.message });
    }
  }
  return results;
}
...
async function testConnection(rootDir, input) {
  ...
  if (input.id) {
    storedAccount = readStoredAccount(rootDir, input.id);
  }
